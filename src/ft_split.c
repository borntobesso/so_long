...
/*   Updated: 2022/02/02 10:49:41 by sojung           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*line_malloc(char *s, t_img *img, char *map, char **res)
{
	...
	if (line == NULL)
		free_all_split(map, res, img, "Problem encountered on malloc.\n");
	i = 0;
	while (*s && *s != '\n')
	{
		line[i] = *(s++);
...
char	**ft_split(char *s, char c, t_img *img)
{
	...
	if (res == NULL)
		ft_error(img, "Problem encountered on malloc\n", 0);
	i = 0;
	j = 0;
	while (s[j])
	{
		...
		if (s[j] && s[j] != c)
		{
			res[i] = line_malloc(&s[j], img, s, res);
			if (res[i++] == NULL)
				free_all_split(s, res, img, "The map is not rectangular.\n");
			while (s[j] && s[j] != c)
				j++;
		}
	}
